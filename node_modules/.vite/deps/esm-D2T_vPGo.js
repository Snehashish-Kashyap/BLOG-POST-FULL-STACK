	...
	constructor(container) {
		...
		this._initTrail = async () => {
			...
			if (trailFill.color) {
				...
				if (!fillColor) return;
				const trail$1 = options.particles.move.trail;
				this._trailFill = {
					color: { ...fillColor },
					opacity: 1 / trail$1.length
				};
...
var EventListeners = class {
	constructor(container) {
		...
		this._handleThemeChange = (e) => {
			const mediaEvent = e, container$1 = this.container, options = container$1.options, defaultThemes = options.defaultThemes, themeName = mediaEvent.matches ? defaultThemes.dark : defaultThemes.light, theme = options.themes.find((theme$1) => theme$1.name === themeName);
			if (theme && theme.default.auto) container$1.loadTheme(themeName);
		};
		this._handleVisibilityChange = () => {
			...
			if (document && document.hidden) {
				...
				if (container$1.getAnimationStatus()) container$1.play(true);
		...
		this._manageInteractivityListeners = (mouseLeaveTmpEvent, add) => {
			const handlers = this._handlers, container$1 = this.container, options = container$1.actualOptions;
			const interactivityEl = container$1.interactivity.element;
			if (!interactivityEl) return;
			const html = interactivityEl, canvasEl = container$1.canvas.element;
		...
		this._manageListeners = (add) => {
			...
			} else if (detectType === "parent" && canvasEl) container$1.interactivity.element = canvasEl.parentElement ?? canvasEl.parentNode;
		...
		this._mouseTouchClick = (e) => {
			...
			for (const [, plugin] of container$1.plugins) {
...
var Particle = class {
	constructor(engine, id, container, position, overrideOptions, group) {
		this.container = container;
		this._calcPosition = (container$1, position$1, zIndex, tryCount = 0) => {
			for (const [, plugin] of container$1.plugins) {
				const pluginPos = plugin.particlePosition !== void 0 ? plugin.particlePosition(position$1, this) : void 0;
			...
			if (this._checkOverlap(pos, tryCount)) return this._calcPosition(container$1, void 0, zIndex, tryCount + 1);
			return pos;
		};
		...
		this._initPosition = (position$1) => {
			const container$1 = this.container, zIndexValue = getRangeValue(this.options.zIndex.value);
			this.position = this._calcPosition(container$1, position$1, clamp(zIndexValue, 0, container$1.zLayers));
...
var QuadTree = class QuadTree {
	constructor(rectangle, capacity) {
		...
		this._subdivide = () => {
			...
			for (let i = 0; i < 4; i++) this._subs.push(new QuadTree(new Rectangle(x + width / 2 * (i % 2), y + height / 2 * (Math.round(i / 2) - i % 2), width / 2, height / 2), capacity$1));
			this._divided = true;
		};
		this._points = [];
		this._divided = false;
...
var Particles = class {
	constructor(engine, container) {
		...
		this._initDensityFactor = (densityOptions) => {
			...
			if (!container$1.canvas.element || !densityOptions.enable) return 1;
...
var Container = class {
	...
	setNoise(noiseOrGenerator, init$1, update) {
		if (!guardCheck(this)) return;
		this.setPath(noiseOrGenerator, init$1, update);
	}
	setPath(pathOrGenerator, init$1, update) {
		if (!pathOrGenerator || !guardCheck(this)) return;
		const pathGenerator = { ...defaultPathGenerator };
		if (isFunction(pathOrGenerator)) {
			pathGenerator.generate = pathOrGenerator;
			if (init$1) pathGenerator.init = init$1;
...
//#endregion
export { AnimatableColor, Circle, OptionsColor, Rectangle, ValueWithRandom, Vector, addEasing, arrayRandomIndex, calcPositionOrRandomFromSize, calcPositionOrRandomFromSizeRanged, calculateBounds, circleBounce, circleBounceDataFromParticle, clamp, colorMix, deepExtend, divMode, divModeExecute, drawLine, drawTriangle, errorPrefix, executeOnSingleOrMultiple, getDistance, getDistances, getEasing, getHslAnimationFromHsl, getLinkColor, getLinkRandomColor, getLogger, getRandom, getRangeMax, getRangeValue, getSize, getStyleFromHsl, getStyleFromRgb, getValue, initParticleNumericAnimationValue, isArray, isDivModeEnabled, isInArray, isNumber, isObject, isPointInside, isSsr, itemFromArray, itemFromSingleOrMultiple, loadFont, loadParticlesOptions, mouseLeaveEvent, mouseMoveEvent, randomInRange, rangeColorToHsl, rangeColorToRgb, rectBounce, rgbToHsl, setRangeValue, tsParticles };
//# sourceMappingURL=esm-D2T_vPGo.js.map
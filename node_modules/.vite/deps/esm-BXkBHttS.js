import { Circle, OptionsColor, Rectangle, ValueWithRandom, Vector, addEasing, calculateBounds, circleBounce, circleBounceDataFromParticle, clamp, colorMix, divMode, divModeExecute, drawLine, drawTriangle, errorPrefix, executeOnSingleOrMultiple, getDistance, getDistances, getEasing, getHslAnimationFromHsl, getLinkColor, getLinkRandomColor, getLogger, getRandom, getRangeMax, getRangeValue, getStyleFromHsl, getStyleFromRgb, getValue, initParticleNumericAnimationValue, isArray, isDivModeEnabled, isInArray, isObject, isPointInside, isSsr, itemFromArray, itemFromSingleOrMultiple, loadFont, mouseLeaveEvent, mouseMoveEvent, randomInRange, rangeColorToHsl, rangeColorToRgb, rectBounce, rgbToHsl, setRangeValue, tsParticles } from "./esm-D2T_vPGo.js";
...
var Attractor$1 = class extends ExternalInteractorBase {
	constructor(engine, container) {
		...
		this._clickAttract = () => {
			...
			if (!container$1.attract) container$1.attract = { particles: [] };
			const { attract } = container$1;
			if (!attract.finish) {
				if (!attract.count) attract.count = 0;
				attract.count++;
				if (attract.count === container$1.particles.count) attract.finish = true;
			}
			if (attract.clicking) {
				const mousePos = container$1.interactivity.mouse.clickPosition, attractRadius = container$1.retina.attractModeDistance;
		...
		this._hoverAttract = () => {
			const container$1 = this.container, mousePos = container$1.interactivity.mouse.position, attractRadius = container$1.retina.attractModeDistance;
			if (!attractRadius || attractRadius < 0 || !mousePos) return;
			this._processAttract(mousePos, attractRadius, new Circle(mousePos.x, mousePos.y, attractRadius));
		};
		this._processAttract = (position, attractRadius, area) => {
			const container$1 = this.container, attractOptions = container$1.actualOptions.interactivity.modes.attract;
			if (!attractOptions) return;
			const query = container$1.particles.quadTree.query(area, (p) => this.isEnabled(p));
...
var Bouncer = class extends ExternalInteractorBase {
	constructor(container) {
		...
		this._processMouseBounce = () => {
			const container$1 = this.container, tolerance = 10 * container$1.retina.pixelRatio, mousePos = container$1.interactivity.mouse.position, radius = container$1.retina.bounceModeDistance;
		...
		this._singleSelectorBounce = (selector, div) => {
			const container$1 = this.container, query = document.querySelectorAll(selector);
	...
	constructor(container) {
		...
		this._clickBubble = () => {
			...
			for (const particle of query) {
				...
				const pos = particle.getPosition(), distMouse = getDistance(pos, mouseClickPos), timeSpent = ((/* @__PURE__ */ new Date()).getTime() - (container$1.interactivity.mouse.clickTime || 0)) / 1e3;
		...
		this._hoverBubble = () => {
			...
			for (const particle of query) {
				...
				if (pointDistance <= distance) {
					if (ratio >= 0 && container$1.interactivity.status === mouseMoveEvent) {
		...
		this._process = (particle, distMouse, timeSpent, data) => {
			...
			const bubbleDuration = bubbleOptions.duration, bubbleDistance = container$1.retina.bubbleModeDistance, particlesParam = data.particlesObj.optValue, pObjBubble = data.bubbleObj.value, pObj = data.particlesObj.value || 0, type = data.type;
		...
		this._singleSelectorHover = (delta, selector, div) => {
			...
			selectors.forEach((item) => {
				const elem = item, pxRatio = container$1.retina.pixelRatio, pos = {
					x: (elem.offsetLeft + elem.offsetWidth / 2) * pxRatio,
					y: (elem.offsetTop + elem.offsetHeight / 2) * pxRatio
				}, repulseRadius = elem.offsetWidth / 2 * pxRatio, area = div.type === "circle" ? new Circle(pos.x, pos.y, repulseRadius) : new Rectangle(elem.offsetLeft * pxRatio, elem.offsetTop * pxRatio, elem.offsetWidth * pxRatio, elem.offsetHeight * pxRatio), query = container$1.particles.quadTree.query(area, (p) => this.isEnabled(p));
				for (const particle of query) {
...
var Pauser = class extends ExternalInteractorBase {
	constructor(container) {
		...
		this.handleClickMode = (mode) => {
			...
			const container$1 = this.container;
			if (container$1.getAnimationStatus()) container$1.pause();
...
var Pusher = class extends ExternalInteractorBase {
	constructor(container) {
		...
		this.handleClickMode = (mode) => {
			...
			const group = itemFromArray([void 0, ...pushOptions.groups]), groupOptions = group !== void 0 ? container$1.actualOptions.particles.groups[group] : void 0;
			container$1.particles.push(quantity, container$1.interactivity.mouse, groupOptions, group);
		};
	}
	clear() {}
	init() {}
	...
	constructor(engine, container) {
		...
		this._clickRepulse = () => {
			...
			if (repulse.clicking) {
				...
				const repulseRadius = Math.pow(repulseDistance / 6, 3), mouseClickPos = container$1.interactivity.mouse.clickPosition;
...
var Linker = class extends ParticlesInteractorBase {
	constructor(container) {
		...
		this._setColor = (p1) => {
			...
			const container$1 = this.linkContainer, linksOptions = p1.options.links;
...
var LinkInstance = class {
	constructor(container) {
		...
		this._drawLinkLine = (p1, link) => {
			...
			const container$1 = this.container, options = container$1.actualOptions, p2 = link.destination, pos1 = p1.getPosition(), pos2 = p2.getPosition();
		...
		this._drawLinkTriangle = (p1, link1, link2) => {
			...
			const container$1 = this.container, options = container$1.actualOptions, p2 = link1.destination, p3 = link2.destination, opacityTriangle = triangleOptions.opacity ?? (link1.opacity + link2.opacity) / 2;
...
//#endregion
export { ExternalInteractorBase, loadSlim };
//# sourceMappingURL=esm-BXkBHttS.js.map